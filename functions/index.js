...
const cors = require('cors')({ origin: true }); // NIEUW: Importeer CORS HIER!
...
const REDIRECT_URI = functions.config().googleapi.redirect_uri; // Deze blijft voor de OAuth2Client
...
// --- Cloud Function 1: getGoogleAuthUrl (NU ALS onRequest) ---
// Deze functie genereert de Google OAuth autorisatie URL.
// We gebruiken onRequest om handmatige CORS-headers te kunnen instellen.
exports.getGoogleAuthUrl = functions.https.onRequest((req, res) => {
  // Gebruik de CORS middleware
  cors(req, res, async () => {
    // Controleer of de gebruiker is geauthenticeerd. Dit is een vereenvoudigde check voor onRequest,
    // in een echte app zou je hier een Firebase Auth token validatie middleware willen.
    // Voor nu gaan we ervan uit dat de frontend alleen aanroept als de gebruiker is ingelogd.
    // Echter, de 'state' parameter zal de UID bevatten voor de volgende stap (saveGoogleDriveTokens).
    const userId = req.query.userId; // Haal userId op uit query parameter van de frontend
...
// --- Cloud Function 2: saveGoogleDriveTokens (blijft onCall) ---
...
// --- Cloud Function 3: uploadFileToGoogleDrive (blijft onCall) ---
